...
docker run -d --name invoice-extractor-backend -p 10.200.7.77:6511:3000 --env-file .env -e SERVER_HOST=10.200.7.77 -v invoice_data:/app/backend/extracted_invoice_data_json -v markdown_data:/app/backend/markdown\ data --restart unless-stopped invoice-extractor-backend:latest

REM Run the frontend container
echo 🚀 Starting Invoice Extractor Frontend...
docker run -d --name invoice-extractor-frontend -p 10.200.7.77:6500:6500 -e VITE_API_BASE_URL=http://10.200.7.77:6511 --restart unless-stopped invoice-extractor-frontend:latest
